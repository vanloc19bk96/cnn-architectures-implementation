...
def alexnet12(input_x):
	# first layer
	x = Conv2D(filters=96, kernel_size=(11, 11), strides=4, use_bias=False, activation="relu", padding="valid")(input_x)
	x = MaxPool2D(pool_size=(3, 3), strides=2)(x)
	x = BatchNormalization()(x)

	# second layer
	x = Conv2D(filters=256, kernel_size=(5, 5), strides=1, use_bias=False, activation="relu", padding="valid")(x)
	...
	x = Conv2D(filters=384, kernel_size=(3, 3), strides=1, use_bias=False, activation="relu", padding="valid")(x)
	x = ZeroPadding2D(padding=(1, 1))(x)
	x = Conv2D(filters=256, kernel_size=(3, 3), strides=1, use_bias=False, activation="relu", padding="valid")(x)
	x = ZeroPadding2D(padding=(1, 1))(x)
	x = MaxPool2D(pool_size=(3, 3), strides=2)(x)
